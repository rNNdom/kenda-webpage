	...
	function update() {
		...
		if(openTime > 0) {
			if(offset-timeLeft < openTime) {
				document.getElementById(eventId).innerHTML = '<span class="event-schedule-open">Abierto</span>';
				timeLeft = timeLeft-1;
				return;
			}
		} else {
			if(duration > 0) {
				if(offset-timeLeft < duration) {
					document.getElementById(eventId).innerHTML = '<span class="event-schedule-inprogress">En curso</span>';
					timeLeft = timeLeft-1;
					return;
				}
			}
